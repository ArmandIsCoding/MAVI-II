...
void Game::DrawGame()
{
	...
	// Dibujar el avatar fijo
	m_fixedAvatar->Dibujar(*wnd);
	m_fixedAvatar->Actualizar(); // to-do: quitar porque es fijo.
}
...
void Game::DoEvents()
{
	...
	while (wnd->pollEvent(evt))
	{
		...
		case Event::KeyPressed:
			...
			{
				controlBodyA->SetLinearVelocity(b2Vec2(controlBodyA->GetLinearVelocity().x, -125.0f));
			}
			...
			{
				controlBodyA->SetLinearVelocity(b2Vec2(-125.0f, controlBodyA->GetLinearVelocity().y));
			}
...
void Game::InitPhysics()
{
	...
	//A:
	controlBodyA = Box2DHelper::CreateRectangularDynamicBody(phyWorld, 5.0f, 5.0f, 9.5, 0.1f, 0.9f);
	controlBodyA->SetTransform(b2Vec2(30.0f, 50.0f), 10.0f);

	//B: Cuerpo fijo.
	b2Body* controlBodyB = Box2DHelper::CreateRectangularStaticBody(phyWorld, 5.0f, 5.0f);
	controlBodyB->SetTransform(b2Vec2(50.0f, 50.0f), 0.0f);
	...
	m_fixedAvatar = new Avatar(controlBodyB, sprite);

	// La empujamos para que rebote desde el principio
	// se puede alterar la trayectoria con las felchas
	controlBodyA->SetLinearVelocity(b2Vec2(controlBodyA->GetLinearVelocity().x, -125.0f));
	controlBodyA->SetLinearVelocity(b2Vec2(125.0f, controlBodyA->GetLinearVelocity().y));

	// Crear un joint entre los dos cuerpos
	b2DistanceJointDef jointDef;
	jointDef.Initialize(controlBodyA, controlBodyB, controlBodyA->GetPosition(), controlBodyB->GetPosition());
	jointDef.damping = 0.5f; // Damping 
	jointDef.stiffness = 0.1f; // Stiffness 
	phyWorld->CreateJoint(&jointDef); // Crear el joint en el mundo físico

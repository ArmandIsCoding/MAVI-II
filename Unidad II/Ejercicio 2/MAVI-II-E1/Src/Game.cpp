...
void Game::InitPhysics()
{
	...
	b2Body* ceilingWallBody = Box2DHelper::CreateRectangularStaticBody(phyWorld, 90, 5);
	ceilingWallBody->SetTransform(b2Vec2(90.0f, 0.0f), 0.0f);

	// Crear un cuerpo dinámico circular
	b2BodyDef bodyDef;
	...
	fixtureDef.friction = 0.2f;  // Baja fricción
	fixtureDef.restitution = 120.0f;  // Mega resitución

	controlBody = phyWorld->CreateBody(&bodyDef);
	controlBody->CreateFixture(&fixtureDef);

	// Asegúrate de que el cuerpo esté despierto
	controlBody->SetAwake(true);
}
